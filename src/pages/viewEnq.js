...
const ViewEnq = () => {
  ...
  const { getAData } = enqState;
  const enqName = getAData?.name;
  const enqMobile = getAData?.mobile;
  const enqEmail = getAData?.email;
  const enqComment = getAData?.comment;
  const enqStatus = getAData?.status;
  useEffect(() => {
